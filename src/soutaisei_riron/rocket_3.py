...
dt = 0.1  # フレームごとの時間ステップ

# 光が宇宙船内を往復する時間（ローレンツ短縮なし）
cycle_time = (2 * L) / c  # 光が1往復する時間
...
# 時計の表示（宇宙船の時計は動くが、地上の時計は固定）
time_text_ship = ax[0].text(0, L * 0.8, "", fontsize=12, ha="center", color="blue")
time_text_ground = ax[1].text(1, L * 0.8, f"時刻: 0.0 s", fontsize=12, ha="center", color="blue")  # 地上時計は固定

# アニメーションの更新関数
def update(frame):
    t = frame * dt  # 経過時間
    phase = (t % cycle_time) / cycle_time  # 光の往復フェーズ（0.0〜1.0）

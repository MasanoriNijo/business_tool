...
def main(username, password, keyword_regex, n_days_ago = 7):
    ...
    # echo コマンドを実行してファイルを空にする
    command2 = ['bash', '-c', 'echo "" > ../out/syuho_all.txt']
    # コマンドの実行
    subprocess.run(command2, check=True)

    # echo コマンドを実行してファイルを空にする
    command3 = ['bash', '-c', 'echo "" > ../out/syuho_matome.txt']
    # コマンドの実行
    ...
    for ind, extracted_text in enumerate(extracted_texts):
        dateRegex = ".*(\d{4}月第\d{1}週).*"
        date = ""
        append_text_to_file("\n" + subjects[ind], f"{config['SRC_TO_OUT_PATH']}/{config['SYUHO_ALL_FILE']}")
        append_text_to_file(extracted_text, f"{config['SRC_TO_OUT_PATH']}/{config['SYUHO_ALL_FILE']}")
...
if __name__ == "__main__":
    ...
    keyword_regex = "^【週報】.*" # 【週報】11月第4週 二條 11月22日(金)～11月28日(木) 週報
...
# 使い方
# python3 shuho_matome.py N (Nは過去何日分 省略した場合90 3か月前)
